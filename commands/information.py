...
class Information(commands.Cog):
    ...
    @commands.command(description="Посмотреть аватарку пользователя или свою аватарку",
                      usage="<Имя пользователя (не обязательно)>")
    async def avatar(self, ctx, member: discord.Member = None):
        if member is None:
            member = ctx.author
            embed = discord.Embed(title=f"Аватарка {member.name}",
                                  description=f"[ссылка на аватарку]({member.avatar_url})", color=member.color)
    ...
    async def about(self, ctx):
        ...
        embed = discord.Embed(title='О боте',
                              description=f'Привет я Странник Бот, я создан для сервера **__{guild}__**. Я '
                                          f'приватный-бот, но если уж хочешь пригласить на свой сервер, '
                                          f'то вот [**ссылка**](https://discord.com/oauth2/authorize?client_id=814877333453799465&permissions=8&scope=bot) на приглашение',
    ...
    async def reg_all_bio(self, ctx, guilds: discord.Guild = None):
        if ctx.author.id == 779381037091782666 or ctx.author.id == 443484756613660674:
            if guilds is None:
    ...
    @commands.command(aliases=["user", "u_i", "info", "юзер", "ю_и", "инфо"],
                      description="Информация о пользователе или о себе", usage="<Имя пользователя (не обязательно)>")
    async def user_info(self, ctx, member: discord.Member = None):
        ...
        if member is None:
            member = ctx.author
            embed = discord.Embed(title=f'Информация о {member.name}#{member.discriminator}',
                                    description=f"Биография: {isbio1()}", color=member.color)
            ...
            embed = discord.Embed(title=f'Информация о {member.name}#{member.discriminator}',
                                    description=f"Биография: {isbio2()}", color=member.color)
            embed.set_author(name=member.name, icon_url=member.avatar_url)
            embed.add_field(name="ID Юзера:", value=member.id)
            embed.add_field(name="Ник на сервере:", value=isnick())
            embed.add_field(name="Присоеденился на сервер:", value=member.joined_at.strftime("%d/%m/%Y"))
    ...
    @commands.command(aliases=["serverinfo", "infoserver", "серверинфо", "сервер", "инфосервер"],
                      description="Информация о сервере", usage="<s.server>")
    async def server(self, ctx):
        ...
        def isafk():
            if ctx.guild.afk_channel:
                return f'{ctx.guild.afk_channel.mention}'
            else:
        ...
        def isboost():
            if ctx.guild.premium_subscription_count:
                ...
                if ctx.guild.premium_subscription_count == 1 or \
                        ctx.guild.premium_subscription_count == 2 or \
                        ctx.guild.premium_subscription_count == 3 or \
                        ctx.guild.premium_subscription_count == 4:
        ...
        def issystemchannel():
            if ctx.guild.system_channel:
                return f'{ctx.guild.system_channel.mention}'
            else:
                return 'Нету'

        def isrules():
            if ctx.guild.rules_channel:
                return f'{ctx.guild.rules_channel.mention}'
            else:
                return 'Нету'
        ...
        embed2.add_field(name="Количество эмодзи:", value=f"{len(ctx.guild.emojis)}/{ctx.guild.emoji_limit + ctx.guild.emoji_limit}")
        ...
        embed3.add_field(name=f"Всего stage каналов:", value=len(ctx.guild.stage_channels))
        ...
        if rec['status']['description'] == "Все системы в рабочем состоянии":
            ...
            color = 0xAA00AA
        embed5 = discord.Embed(title=rec['status']['description'], colour=color,
        ...
        if arg == "rename":
            if member == None:
                ...
                embed = discord.Embed(title="Успешно!",
                                      description=f"Успешно изменилась биография {ctx.author.name}. Текст биографии: {self.bios.find_one({'_id': ctx.author.id})['bio']}",
                                      colour=discord.Colour.green())
                await ctx.send(embed=embed)
            else:
                return
            ...
            if member == None:
                ...
                embed = discord.Embed(title="Успешно!", description=f"Успешно Удалилась биография {ctx.author.name}",
                                      colour=discord.Colour.green())
                await ctx.send(embed=embed)
        elif arg == "create":
            def colladd():
            ...
            if member == None:
                ...
                self.bios.update_one({"_id": ctx.author.id, "server": ctx.guild.id}, {"$set": {"bio": " ".join(args)}})
                embed = discord.Embed(title="Успешно!",
                                      description=f"Успешно создалась биография {ctx.author.name}. Текст биографии: {self.bios.find_one({'_id': ctx.author.id})['bio']}",
                                      colour=discord.Colour.green())
                await ctx.send(embed=embed)
