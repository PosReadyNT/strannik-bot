...
class Music(commands.Cog):
    ...
    async def pause(self, ctx):
        ...
        if not player.is_playing:
            return await ctx.send('Песня не может быть на паузе, т.к нету песни или песня уже на паузе', delete_after=15)
    ...
    async def leave_servers(self, ctx):
        for guild in self.bot.guilds:
            ...
            await player.disconnect()
        await ctx.send("Я ушёл со всех голосовых каналов")
    ...
    async def queue(self, ctx):   
        ...
        if not player.current or not controller.queue._queue:
            return await ctx.send('Нету списка', delete_after=20)
 
        upcoming = list(itertools.islice(controller.queue._queue, 0, 5))
 
        fmt = '\n'.join(f'**`{str(song)}`**' for song in upcoming)
        embed = discord.Embed(title=f'Список песен: {len(upcoming)}', description=fmt)
