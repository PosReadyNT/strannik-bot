...
async def chess_loop(user1, user2, ctx, bot):
    ...
    #cairosvg.svg2png(url="chess_board.svg", write_to="chess.png")
            ...
            try:
                ...
                if move in board.legal_moves:
                    ...
                    embed = discord.Embed(title=f"Move",
                                            description=f"{ctx.author.mention} moved {move_from} to {move_to}!",
                                            color=discord.Color.green())
                    await ctx.send(embed=embed)
                    ...
                    embed = discord.Embed(title=f"Ошибка",
                                            description=f"Вы ввели не так ход. Пример: `b2, b4` Пишите мне в ЛС! "
